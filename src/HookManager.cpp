...
namespace HookManager {
    ...
    using fnVirtualAlloc = LPVOID(WINAPI*)(LPVOID, SIZE_T, DWORD, DWORD);
    ...
    fnVirtualAlloc g_OriginalVirtualAlloc = nullptr;
    fnVirtualAllocEx g_OriginalVirtualAllocEx = nullptr;
    ...
    void ResetOriginalFunctions() {
        g_OriginalVirtualAlloc = nullptr;
        g_OriginalVirtualAllocEx = nullptr;
    ...
    LPVOID WINAPI HookedVirtualAlloc(LPVOID lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD flProtect) {
        ...
        if (result != nullptr) {
            Logger::Log(L"[Hook] VirtualAlloc called. Base: " + ToHex((uintptr_t)result) +
                        L", Size: " + ToHex(dwSize) +
                        L", Protect: " + ToHex(flProtect));

            // Если выделяется исполняемая память или память с правами на чтение/запись (куда потом могут записать PE и сменить права)
            if (ShouldScanProtection(flProtect)) {
                MemoryScanner::QueueScan(result, dwSize);
            }
        }
        return result;
    }

    // Наш хук для VirtualAllocEx
    LPVOID WINAPI HookedVirtualAllocEx(HANDLE hProcess, LPVOID lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD flProtect) {
        if (g_InsideHook) {
            return g_OriginalVirtualAllocEx(hProcess, lpAddress, dwSize, flAllocationType, flProtect);
        }

        HookCallGuard guard;
        LPVOID result = g_OriginalVirtualAllocEx(hProcess, lpAddress, dwSize, flAllocationType, flProtect);

        // Проверяем, выделяется ли память в текущем процессе
        if (result != nullptr && (hProcess == GetCurrentProcess() || GetProcessId(hProcess) == GetCurrentProcessId())) {
            Logger::Log(L"[Hook] VirtualAllocEx called. Base: " + ToHex((uintptr_t)result) +
                        L", Size: " + ToHex(dwSize) +
                        L", Protect: " + ToHex(flProtect));
    ...
    BOOL WINAPI HookedVirtualProtect(LPVOID lpAddress, SIZE_T dwSize, DWORD flNewProtect, PDWORD lpflOldProtect) {
        ...
        if (result) {
            ...
            // Если права меняются на исполняемые
            if (ShouldScanProtection(flNewProtect)) {
                MemoryScanner::QueueScan(lpAddress, dwSize);
            }
        }
        return result;
    }

    // Наш хук для VirtualProtectEx
    ...
    bool InitializeHooks() {
        ...
        bool success =
            CreateApiHook("VirtualAlloc", reinterpret_cast<LPVOID>(HookedVirtualAlloc), &g_OriginalVirtualAlloc) &&
            CreateApiHook("VirtualAllocEx", reinterpret_cast<LPVOID>(HookedVirtualAllocEx), &g_OriginalVirtualAllocEx) &&
